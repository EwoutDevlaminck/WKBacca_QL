...
import CommonModules.LoadneTeField as neTefile
...
class TokamakEquilibrium2(MagneticSurfaces):
    ...
    def __setup__(self, idata):
        ...
        if idata.analytical_tokamak == 'No':
            ...
            neTedata = neTefile.read(input_dir)
            Rloc, zloc, neloc, Teloc = neTedata
            Rgrid1D = Rloc[:,0]
            zgrid1D = zloc[0,:]
            self.NeInt = bispl.BiSpline(Rgrid1D, zgrid1D, neloc)
            self.TeInt = bispl.BiSpline(Rgrid1D, zgrid1D, Teloc)
